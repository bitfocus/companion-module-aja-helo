...
state = {
    **state,
    **{
        f"eParamID_{t}ProfileName_{i}": f"{t} Profile Name {i}"
        for i in range(1, 11)
        for t in ["Streaming", "Recording"]
    },
}
...
def config():
    ...
    if action == "set":
        ...
        if paramId == "eParamID_FilenamePrefix" or paramId in [
            f"eParamID_{t}ProfileName_{i}"
            for i in range(1, 11)
            for t in ["Streaming", "Recording"]
        ]:
            state[paramId] = value
